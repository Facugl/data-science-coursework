...
\begin{tikzpicture}[
    ...
    % Estilo de nodos del grafo (separado del estilo global)
    \tikzset{
    ...
    \node[vertice] (2) at (3, 0)     {2};
    \node[vertice] (1) at (4.5,-1) {1};
    \node[vertice] (4) at (1.2,-1.8) {4};
    \node[vertice] (6) at (0, -3.6)  {6};
    \node[vertice] (3) at (3, -3.6)  {3};

    % Aristas
    \draw (5) to node[peso, above]       {10} (2);
    \draw (2) to[bend left=15]  node[peso, above right] {2} (1);
    \draw (1) to[bend left=15]  node[peso, below left]  {20}  (2);
    \draw (2) to node[peso, above left]  {30} (4);
    \draw (2) to node[peso, right]       {4}  (3);
    \draw (4) to node[peso, above right]  {15} (5);
    \draw (4) to node[peso, below right]        {4}  (6);
    \draw (6) to node[peso, left]        {10} (5);
    \draw (6) to node[peso, below]       {10} (3);
    \draw (3) to node[peso, below right] {5}  (1);
\end{tikzpicture}
...
\begin{itemize}[leftmargin=2cm]
    \item \textbf{Dirigido o no dirigido:} Es un grafo \textbf{dirigido}, ya que se puede observar que cada arista posee un sentido definido.

    \item \textbf{Conexo:} El grafo \textbf{no es conexo}. Existen ciertos nodos 
    a los cuales no se puede acceder desde cada nodo del grafo siguiendo la 
    dirección de las aristas.

    \item \textbf{Ponderado:} El grafo \textbf{es ponderado}, ya que se puede observar que cada arista 
    tiene asociado un peso numérico.

    \item \textbf{Maximal / Minimal:} El grafo \textbf{no posee ningún nodo maximal}. 
    Sin embargo, \textbf{sí posee nodos minimales}; por ejemplo, el nodo~4, 
    del cual no parte ninguna arista de salida.
\end{itemize}
...
donde $A_{ij} = 1$ indica que existe una arista dirigida desde el nodo $i$ 
hacia el nodo $j$, y $A_{ij} = 0$ indica su ausencia.
...
\textit{a) Al encontrarse un elemento par, deberá generar y mostrar una pila, caso distinto, 
al encontrarse con un elemento impar, deberá generar y mostrar una cola resultante conteniendo 
los números impares.}

\textit{a) Mostrar el promedio de los elementos de la pila P1.}

\solucion

El algoritmo recorre primero la lista completa para validar que todos los
elementos sean números enteros. Si encuentra algún elemento inválido, informa
el error y detiene la ejecución antes de reservar memoria para las estructuras
auxiliares. Una vez validada la lista, la recorre nuevamente: por cada elemento
par lo apila en una \texttt{pila} y por cada elemento impar lo encola  una \texttt{cola}.
...
\begin{itemize}[leftmargin=2cm]
    ...
    \textbf{No es completo}, dado que el nodo~1 no posee hijos.
